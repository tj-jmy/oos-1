...
void Exception::PageFault(struct pt_regs *regs, struct pte_context *context)
{
	...
	// 如果error_code的第1位为0，表示是写操作引起的缺页异常
	Diagnose::Write("Page Fault at %x, error_code: %x\n", cr2, context->error_code);
	// Diagnose::Write("pid: %d\n", current->p_pid);
	if ((context->error_code & 0x2)) // 读写异常
	{
		// 从当前 PTE 中读出物理页框号 base
		unsigned int base = md.m_UserPageTableArray[cr2 >> 22].m_Entrys[(cr2 >> 12) & 0x3ff].m_PageBaseAddress;
		unsigned int *pageRefCnt = Kernel::Instance().GetUserPageManager().GetPageRefCnt();
		// Diagnose::Write("dir: %x, table: %x, base: %x, refCnt: %d\n", cr2 >> 22, (cr2 >> 12) & 0x3ff, base, pageRefCnt[base]);
		if (pageRefCnt[base] == 1) // 如果引用计数为1，说明只有一个进程在使用这个页面，可以直接写
		{
			md.m_UserPageTableArray[cr2 >> 22].m_Entrys[(cr2 >> 12) & 0x3ff].m_ReadWriter = 1;
		}
		else if (pageRefCnt[base] > 1) // 如果引用计数大于1，说明有多个进程在使用这个页面，需要复制一份
		{

			unsigned int newBase = Kernel::Instance().GetUserPageManager().AllocMemory(PageManager::PAGE_SIZE) >> 12;
			for (int i = 0; i < PageManager::PAGE_SIZE; i++)
			{
				Utility::CopySeg(base << 12 | i, newBase << 12 | i);
			}
			md.m_UserPageTableArray[cr2 >> 22].m_Entrys[(cr2 >> 12) & 0x3ff].m_PageBaseAddress = newBase;
			md.m_UserPageTableArray[cr2 >> 22].m_Entrys[(cr2 >> 12) & 0x3ff].m_ReadWriter = 1;
			pageRefCnt[base]--;
		}
		md.MapToPageTable();
