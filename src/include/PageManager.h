...
public:
	...
	static const unsigned int TOTAL_PAGES = 0x10000000 / PAGE_SIZE; /* 1M物理内存共有256页 */
...
private:
	...
	bool bit_is_set(unsigned long index) const;
	void set_bit(unsigned long index);
	void clear_bit(unsigned long index);
	/* Members */
public:
	unsigned char m_PageBitmap[PageManager::TOTAL_PAGES / 8];

	// 物理页框的引用计数
	unsigned int m_PageRefCnt[PageManager::TOTAL_PAGES];

	unsigned long m_StartAddressIdx;
